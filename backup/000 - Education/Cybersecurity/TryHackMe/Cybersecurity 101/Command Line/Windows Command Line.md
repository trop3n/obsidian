...
- **Automation**: While you can automate GUI tasks, creating a batch file or script with the command you need to repeat is much easier.
- **Remote Management**: CLI makes it very consistent to use SSH to manage a remote system such as a server, router or an IoT device. This approach works well on slow network speeds and systems with limited resources.
## Learning Objectives
...
Before issuing commands, we should note that we can only issue commands with the Windows Path. You can issue the command `set` to check your path from the command line. The terminal output below shows the path where MS Windows will execute commands, as indicated by the line starting with `Path=`.
...
First, you can pipe it through `more` if the output is too long. Then, you can view it page after page by pressing the spacebar button. To demonstrate this, try running `driverquery` and compare it with running `driverquery | more`. In the latter, you can display the output page by page and you can exit is using `CTRL + C`. 
...
Let's find out if we reach `example.com`. In the terminal output below, we can see that we have successfully received four replies. Furthermore, we got some statistics; for instance, the average round trip time is 78ms.
...
- `-n` uses a numerical form for addressed and port numbers

We combine these four options and execute the `netstat -abon` command. The result is quite long, but we display the first few lines in the terminal below. It is clear now that the executable `sshd.exe` is responsible for listening for incoming connections on port 22, as shown in the first line. We can also the process ID (PID) associated with each connection.
...
Similarly, you can move files using the `move` command. An example is shown in the terminal below:
